...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($action == "update") {
        ...
        $delivery_boy = get_safe_value($con, $_POST["delivery_boy"]);

        if ($status == "") {
            $arr = array("status" => "error", "msg" => "Order status is required");
        } elseif ($delivery_boy == "") {
            $arr = array("status" => "error", "msg" => "Order delivery boy is required");
        } else {
            ...
            if ($result->num_rows > 0) {
                $query = "UPDATE orders SET order_status = ?, delivery_boy_id = ? WHERE id = ?";
                $stmt = $con->prepare($query);
                $stmt->bind_param('sii', $status, $delivery_boy, $id);
